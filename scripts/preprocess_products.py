...
FORBIDDEN_TITLE_KEYWORDS = ["rug", "pillow", "mobile cover", "Stool", "Silla", "Sloane", "Executive", "Stone & Beam"]
...
def title_uses_latin(title: Optional[str], min_latin: int = 1) -> bool:
    """Return True if the title contains at least `min_latin` Latin letters.

    We count alphabetic characters whose Unicode name contains 'LATIN'.
    Titles with fewer than `min_latin` Latin letters are considered
    non-Latin for the purposes of filtering.
    """
    if not title:
        return False
    count = 0
...
def process_product_entry(product: dict, json_path: str, image_base_folder: str, idx: int, debug: bool = False, interactive: bool = False, category_counts: Optional[dict] = None) -> Optional[dict]:
    ...
    # Discard products whose title does not use enough Latin characters
    if not title_uses_latin(item_name, min_latin=5):
        if debug:
            print(f"Discarding product {item_name}: title does not use Latin characters")
        return None
